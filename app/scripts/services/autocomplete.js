...
/*
 *  Takes care of auto-complete in the editor
*/
PhonicsApp.service('Autocomplete', function Autocomplete(Editor) {
  var Range = ace.require('ace/range').Range;

  /*
   * Initialize the autocomplete when editor is ready
  */
  Editor.ready(function (EditorService, editor) {
    autocomplete.init(editor);
  });
  ...
  var autocomplete = {
    ...
    init: function (editor) {
      ...
      // this.testSimpleMatchScorer();
      // this.testFuzzyMatcher();
    },

    suggest: function () {
      var that = this;

      var cursorPosition = this.getEditor().getCursorPosition();
    ...
    updateSuggestions: function (position) {
      ...
      var session = this.getEditSession();
      ...
      $.each(suggestions, function (index, suggestion) {
        ...
        var indexes = that.getMatchIndexes(prefix, suggestion);
    ...
    show: function () {
      ...
      popup.css({
        top: this.computeTopOffset(),
        left: this.computeLeftOffset(),
    ...
    getSelectedSuggestion: function () {
      ...
      if (selectedSuggestion.length < 1) {
        console.warn(('No suggestion selected. Might be a bug.'));
      } else if (selectedSuggestion.length > 1) {
        console.warn(('More than one suggestions selected. Might be a bug.'));
      }
    ...
    onDocumentChange: function (e) {
      ...
      if (e.data.action === 'insertText') {
        ...
        console.warn('Unkown document change action.');
      }
    ...
    addKeyboardCommands: function () {
      var that = this;
      var commandManager = this.getEditor().commands;
    ...
    computeSimpleMatchScore: function (prefix, suggestion) {
      ...
      var localSuggestion = this.isLowerCase(prefix) ? suggestion.toLowerCase() : suggestion;
    ...
    testSimpleMatchScorer: function () {
      var prefix = 'myprefix';
      var suggestion = 'myprefixisshort';
      var score = this.computeSimpleMatchScore(prefix, suggestion);
      if (score !== 8) {
        console.warn('testSimpleMatchScorer lowercase test failed.');
      }
      ...
      if (score !== 8) {
        console.warn('testSimpleMatchScorer uppercase test failed.');
      }
      ...
      score = this.computeSimpleMatchScore(prefix, suggestion);
      if (score !== 2) {
        console.warn('testSimpleMatchScorer mixed case vs. lowercase test failed.');
      }
      ...
      score = this.computeSimpleMatchScore(prefix, suggestion);
      if (score !== 10) {
        console.warn('testSimpleMatchScorer mixed case test failed.');
      }
    },

    testFuzzyMatcher: function () {
      var isMatching = this.isMatchingFuzzily('mlf', 'mylongfunctionname');
      if (!isMatching) {
        console.warn('testFuzzyMatcher mlf vs. mylongfunctionname failed.');
      }

      isMatching = this.isMatchingFuzzily('mLn', 'myLongFunctionName');
      if (!isMatching) {
        console.warn('testFuzzyMatcher mLn vs. myLongFunctionName failed.');
      }
      isMatching = this.isMatchingFuzzily('mLFuny', 'myLongFunctionName');
      if (isMatching) {
        console.warn('testFuzzyMatcher mLFuny. myLongFunctionName failed.');
      }
    }
  };
